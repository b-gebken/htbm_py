...
A simple Python implementation of a trust-region bundle method using higher-order cutting-plane models (HTBM) for the solution of nonsmooth, nonconvex optimization problems. The trust-region subproblems are solved via the Python wrapper _cyipopt_ (https://github.com/mechmotum/cyipopt) for _IPOPT_ (https://github.com/coin-or/Ipopt). The original Matlab implementation can be found at https://github.com/b-gebken/higher-order-trust-region-bundle-method, and the corresponding research articles are 
...
- For the HTBM, we use the same parameters as in [GU2026a], Section 6, with $\varepsilon_{thr} = 10^{-6}$ as the smallest TR radius. Since we are only concerned with _local_ convergence and the _local_ nonsmooth structure, we choose an initial TR radius of $10^{-2}$ and an initial point $x^1$ whose distance (in the maximum norm) to an approximated minimum (x_min in the code) is at most $10^{-3}$. (The approximated minima were computed beforehand through a combination of DGS, BFGS and HTBM. Note that due to nonconvexity, we cannot guarantee that these are actually the global minima and not just local minima.) The best point found by the specific run of the HTBM in an example is denoted by $x^{{best}}$. (For analyzing the speed of convergence, we will plot the values $\log_{10}(\\| x^j - x^{{best}} \\|_2)$. Since this value is not defined when $x^j = x^{{best}}$, a vertical dashed line is plotted for such $j$ instead.) To make the behavior of the HTBM as easy to interpret as possible, we do not use random initial points for the bundle and do not reuse bundle information from memory (cf. [GU2026a], Remark 4.2(a)).
...
Before interpreting the results, we first explain our visualization:
- The top-left plot shows the convergence behavior of $(x^j)\_j$ w.r.t. to the index $j$. By the theory of [GU2026a], under certain assumptions, this sequence converges locally R-superlinearly to a minimum. The red line shows $\log_{10}(\varepsilon\_j)$, which is the theoretical upper bound for the black values (cf. [GU2026a], Thm. 4.4).
...
- The bottom-left plot shows, for each iteration of the HTBM, the size of the bundle that was required to build a "sufficiently good" higher-order cutting-plane model. If this size is 1, then HTBM simply solved the subproblem of the classic trust-region Newton method to obtain the next iterate.
...
- Finally, the bottom-right plot shows, for each iterate $x^j$, the fraction $\frac{f(x^j) - f(x^{{best}})}{\\| x^j - x^{{best}} \\|\_2^2}$ plotted against $\log_{10}(\\| x^j - x^{{best}} \\|\_2)$. This can be used to get a rough idea of whether the objective function has quadratic growth around $x^{{best}}$. However, since the minimum may not be unique and may even lie inside a continuous set of minima, this plot has limited meaning. 
...
To analyze the structure of this nonsmoothness, we use the deterministic gradient sampling mechanism of the DGS (https://github.com/b-gebken/DGS). Given any point $x$, this mechanism finds either a descent direction at $x$ or a set $W$ of subgradients (sampled in an $\varepsilon$-ball around $x$) for which a vanishing convex combination exists. By applying it for $x = x^{{best}}$, it yields the latter (since this point is almost a minimum), and the number of subgradients required for this convex combination can be seen as a measure of the complexity of the nonsmoothness. The results are shown in Fig. 6.
...
- The first plot can be used to analyze whether $f$ is a lower-Ck function. For such functions and for any $x, v \in \mathbb{R}^n$, the directional derivative satisfies $df(x,v) = \max_{\xi \in \partial f(x)} \langle \xi, v \rangle$ (see Thm. 10.31 in [Rockafellar, Wets (1998)]). As a numerical approximation of this relationship, for each element $g \in W$ (on the horizontal axis), we check whether a finite difference approximation of $df(x^{{best}},g)$ (red) is greater or equal to $\max_{\xi \in W} \langle \xi, g \rangle$ (blue). The "violation" refers to the difference of the latter minus the former. If this violation is close to zero, then the above equality holds approximately (for this direction $v = g$). Otherwise, the violation being negative means that a subgradient may be missing from the set $W$. Since some subgradients may not be needed to achieve a vanishing convex combination, this does not contradict the above equality. However, if the violation is positive (and not close to zero), then the above equation cannot hold (since adding more subgradients to $W$ would only further increase $\max_{\xi \in W} \langle \xi, g \rangle$), so the function cannot be lower-Ck.
